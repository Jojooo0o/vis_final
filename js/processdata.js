...
function processData() {
  ...
  d3.queue()
    ...
    .awaitAll(function (error, d) {
      ...
      var dayly = calcMinMaxValues(d);
      var yearly = calcMinMaxValues(year_data);
      console.log(dayly);
      console.log(yearly);
      // first drawing outside the loop to give different bool
      drawLineGraph(d[0], year_data[0], false, lineColors[0], dayly);
      for(var i = 1; i < d.length; ++i) {
        drawLineGraph(d[i], year_data[i], true, lineColors[i], dayly);
      }
...
function calcMinMaxValues(data) {
